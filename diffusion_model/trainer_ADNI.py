...
from einops import rearrange # to re-order multidimensional tensors
...
try:
    from apex import amp # apex for mixed precision training?
    APEX_AVAILABLE = True
    print("APEX: ON")
except:
...
def exists(x): # check if x is not None
    return x is not None

def default(val, d): # return default value if val is None
    if exists(val):
        return val
    return d() if isfunction(d) else d

def cycle(dl): # cycle the dataloader, infinite loop over the dataset
    while True:
        for data in dl:
            yield data

def num_to_groups(num, divisor): # split num into groups of size divisor
    groups = num // divisor
...
def loss_backwards(fp16, loss, optimizer, **kwargs): # backward pass with optional mixed precision
    if fp16:
        with amp.scale_loss(loss, optimizer) as scaled_loss:
            scaled_loss.backward(**kwargs) # backward pass with apex mixed precision
    else:
        loss.backward(**kwargs) # regular backward pass: from torch documentation

# small helper modules

# exponential moving average class: to maintain a smoothed version of the model weights
class EMA():  
    def __init__(self, beta):
        super().__init__()
        self.beta = beta

    def update_model_average(self, ma_model, current_model): # update the moving average model
        for current_params, ma_params in zip(current_model.parameters(), ma_model.parameters()):
            old_weight, up_weight = ma_params.data, current_params.data
            ma_params.data = self.update_average(old_weight, up_weight)

    def update_average(self, old, new): # compute the updated average
        if old is None:
            return new
...
def extract(a, t, x_shape): # extract values from a tensor at indices t and reshape to x_shape
    b, *_ = t.shape
    out = a.gather(-1, t)
    return out.reshape(b, *((1,) * (len(x_shape) - 1)))

def noise_like(shape, device, repeat=False): # generate noise tensor of given shape on specified device
    repeat_noise = lambda: torch.randn((1, *shape[1:]), device=device).repeat(shape[0], *((1,) * (len(shape) - 1)))
    noise = lambda: torch.randn(shape, device=device)
    return repeat_noise() if repeat else noise()

def cosine_beta_schedule(timesteps, s = 0.008): # cosine schedule to compute betas for diffusion process
    """
    cosine schedule
...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        alphas = 1. - betas # compute alphas from betas to define diffusion process
        alphas_cumprod = np.cumprod(alphas, axis=0) # cumulative product of alphas over time steps
        alphas_cumprod_prev = np.append(1., alphas_cumprod[:-1]) # previous cumulative product of alphas

        timesteps, = betas.shape # number of diffusion steps
        self.num_timesteps = int(timesteps) # total diffusion steps
        self.loss_type = loss_type # loss type: L1 or L2 = mean absolute error or mean squared error

        to_torch = partial(torch.tensor, dtype=torch.float32) # numpy arr to torch tensor conversion helper

        # register buffers for diffusion parameters = precomputed tensors stored in the model
    ...
    def q_mean_variance(self, x_start, t, c=None): # compute mean and variance of q(x_t | x_0)
        x_hat = 0
    ...
    def predict_start_from_noise(self, x_t, t, noise, c=None): # predict x_0 from x_t and noise
        x_hat = 0.
        return (
    ...
    def q_posterior(self, x_start, x_t, t, c=None): # compute mean and variance of q(x_{t-1} | x_t, x_0)
        x_hat = 0.
        posterior_mean = (
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool, c = None, y=None): # compute mean and variance of p(x_{t-1} | x_t)
        # added Y for diagnosis condition
        if self.with_condition:
            x_recon = self.predict_start_from_noise(x, t=t, noise=self.denoise_fn(torch.cat([x, c], 1), t, y=y))
        else:
    ...
    @torch.no_grad() # @torch.no_grad() decorator to disable gradient calculation
    def p_sample(self, x, t, condition_tensors=None, diagnosis=None, clip_denoised=True, repeat_noise=False): # sample x_{t-1} from x_t
        b, *_, device = *x.shape, x.device
        # added diagnosis condition as y
        model_mean, _, model_log_variance = self.p_mean_variance(x=x, t=t, c=condition_tensors, y=diagnosis, clip_denoised=clip_denoised)
    ...
    @torch.no_grad()
    def p_sample_loop(self, shape, condition_tensors = None, diagnosis=None): # sample x_{t-1} from x_t
        ...
        for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            if self.with_condition:
                ...
                # added diagnosis condition
                img = self.p_sample(img, t, condition_tensors=condition_tensors, diagnosis=diagnosis)
            else:
                img = self.p_sample(img, torch.full((b,), i, device=device, dtype=torch.long))
        return img

    @torch.no_grad()
    def sample(self, batch_size = 2, condition_tensors = None, diagnosis=None): # sample from the model
        # added diagnosis condition
        image_size = self.image_size
        depth_size = self.depth_size
        channels = self.channels
        # added diagnosis condition
        #return self.p_sample_loop((batch_size, channels, depth_size, image_size, image_size), condition_tensors = condition_tensors, diagnosis=diagnosis)
        return self.p_sample_loop((batch_size, channels, image_size, image_size, depth_size), condition_tensors = condition_tensors, diagnosis=diagnosis)

    @torch.no_grad()
    def interpolate(self, x1, x2, t = None, lam = 0.5): # interpolate between two images x1 and x2 at time step t
        b, *_, device = *x1.shape, x1.device 
        t = default(t, self.num_timesteps - 1)

        assert x1.shape == x2.shape # ensure both images have the same shape
    ...
    def q_sample(self, x_start, t, noise=None, c=None): # sample x_t from x_0
    ...
    def p_losses(self, x_start, t, condition_tensors = None, diagnosis=None, noise = None): # compute loss between predicted and true noise
        b, c, h, w, d = x_start.shape
        noise = default(noise, lambda: torch.randn_like(x_start))

        if self.with_condition: # added condition tensors:ADNI
            x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
            # added condition tensors y=diagnosis
            x_recon = self.denoise_fn(torch.cat([x_noisy, condition_tensors], 1), t, y=diagnosis)
        else:
            x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
    ...
    def forward(self, x, condition_tensors=None, diagnosis=None, *args, **kwargs): # forward pass to compute loss
        #b, c, d, h, w, device, img_size, depth_size = *x.shape, x.device, self.image_size, self.depth_size
        b, c, h, w, d, device, img_size, depth_size = *x.shape, x.device, self.image_size, self.depth_size
        ...
        return self.p_losses(x, t, condition_tensors=condition_tensors, diagnosis=diagnosis, *args, **kwargs)
    ...
    def train(self):
        ...
        while self.step < self.train_num_steps:
            ...
            if self.step != 0 and self.step % self.save_and_sample_every == 0:
                ...
                sampleImage = sampleImage[0, 0]  # Extract first batch, first channel
